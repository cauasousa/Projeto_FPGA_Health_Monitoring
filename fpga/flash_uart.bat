...
if not exist "teclado.sv" (
    echo ❌ ERRO: Arquivo teclado.sv não encontrado!
    pause
    exit /b 1
)

if not exist "fsm_menu.sv" (
    echo ❌ ERRO: Arquivo fsm_menu.sv não encontrado!
    pause
    exit /b 1
)
...
yosys -p "read_verilog -sv uart_tx.sv uart_rx.sv uart_top.sv teclado.sv fsm_menu.sv %TOP%.sv; synth_ecp5 -top %TOP% -json %TOP%.json"
